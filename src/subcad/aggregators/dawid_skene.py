...
class DawidSkene:
    r"""Dawid-Skene label aggregator for a crowdsourced dataset.

    Estimates task labels via the Dawid-Skene Expectation-Maximization
    (EM) algorithm, where each worker is modeled by a confusion matrix.
    Majority voting (`MajorityVoting`) is used to initialize EM.
    ...
    !!! Example
        ...
        data_dir = Path(".")
        response_mat, gt_labels = subcad.data.fetch_rte(data_dir, return_X_y=True)

        from subcad.aggregators import DawidSkene
        labels_hat = DawidSkene().fit_predict(response_mat)
        ```
    ...
    tol
        Tolerance used for convergence. At each EM iteration, the changes
        in model parameters and task classification probabilities are
        calculated. If the change is smaller than `tol`, EM is deemed as
        converged.
    ...
    def fit(self, response_mat: npt.NDArray, y=None) -> "DawidSkene":
        """Estimate task labels via the Dawid-Skene EM algorithm.
        ...
        onehot_labels = []
        confusion_mats = []
        mv_labels = MajorityVoting().fit_predict(response_mat)
